...
- **Every chart is the pattern it is named.** The teaching text and the chart generator were written separately, and nothing forced them to agree — so a "major trend reversal" could be drawn with no trend in it to reverse. Each context is now checked against the structural claims its own text makes: a reversal shows a trend with lower highs before it reverses, a two-legged pullback shows an established trend and two legs, a coil narrows, barbwire overlaps. The checks run over every context and both mirrors, and are what stop the prose and the picture drifting apart again.
