...
- **Bar counting is a bar count, not a leg count.** A High 2 is the second bar to trade above the prior bar's high since the pullback began, where a fresh attempt only counts once the pullback has made a new low in between. The second leg does *not* have to trade below the first leg's low. The app computes the count off the actual bars with this rule, so the answer is whatever the tape shows — sometimes a High 1, sometimes a High 3.
