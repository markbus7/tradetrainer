...
**And the target says where it came from.** A target beyond every high and low on the screen is unreadable — you can see the level but not what put it there. When it is a measured move, the measurement is drawn: the leg that was measured, and that same height stood up again from the point it is projected off. The leg carries its height and the projection repeats it — *measured leg 0.95*, *same 0.95 from here* — because the two are often tens of bars apart and "measured move" on its own does not say measured off what. In words too, with the arithmetic — *the leg from 23,245 to 22,204 is 1,041 tall, and that height from 23,256 gives 22,215*. When the target is instead a level already on the chart — a range edge, a wedge's origin — it says so, because that is a different kind of claim.
